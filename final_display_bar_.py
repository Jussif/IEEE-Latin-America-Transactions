...
class Display_Consumption(object):
    ...
    def Error_Bar(self):
        ...
        for idx, val in enumerate(values):
            ...
            xlim = plt.xlim()  # Returns a tuple (min_x, max_x)
            min_x, max_x = xlim
            # Calculate the middle point
            middle_x = (min_x + max_x) / 2
            # Set the y-coordinate for the text (adjust as needed)
            y_text = -62  # Position the text on the y-axis value

            plt.text(middle_x, y_text, "Scenarios", ha='center', fontsize=self.my_figure['TAMANHO_FONTE']-10)
            plt.xticks(rotation=30, fontsize=10) #self.my_figure['TAMANHO_FONTE']-10)
